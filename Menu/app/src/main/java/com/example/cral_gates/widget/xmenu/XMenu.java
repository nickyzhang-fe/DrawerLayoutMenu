package com.example.cral_gates.widget.xmenu;
...
public class XMenu extends RelativeLayout {
	...
	private void init(Context context, AttributeSet attrs) {
			...
			if (null == leftShadowDrawable) {
				leftShadowDrawable = new GradientDrawable(
						GradientDrawable.Orientation.LEFT_RIGHT, new int[] {
								Color.TRANSPARENT, Color.argb(99, 0, 0, 0) });
			}
			mContentView.setLeftShadowDrawable(leftShadowDrawable);
			int touchModeAbove = a.getInt(R.styleable.XMenu_touchModeAbove,
					TOUCHMODE_MARGIN);
			setTouchModeAbove(touchModeAbove);
			// int edgeWidth = a.getInt(R.styleable.XMenu_edgeWidth,
		...
		int mMarginThreshold = (int) TypedValue.applyDimension(
